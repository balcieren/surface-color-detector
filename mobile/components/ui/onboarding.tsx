...
export function Onboarding({
  ...
  // Shared values for gesture blocking (to avoid stale closures)
  const canSwipeNext = useSharedValue(!disableNextButton);
  const canSwipeBack = useSharedValue(!disableBackButton);

  // Update shared values when props change
  React.useEffect(() => {
    canSwipeNext.value = !disableNextButton;
  }, [disableNextButton]);

  React.useEffect(() => {
    canSwipeBack.value = !disableBackButton;
  }, [disableBackButton]);
  ...
  const panGesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
      // Block left swipe (next) if disabled
      if (!canSwipeNext.value && event.translationX < 0) {
        translateX.value = 0;
        return;
      }
      // Block right swipe (back) if disabled
      if (!canSwipeBack.value && event.translationX > 0) {
        translateX.value = 0;
        return;
      }
    ...
    .onEnd((event) => {
      ...
      if (shouldSwipe) {
        if (translationX > 0 && !isFirstStep && canSwipeBack.value) {
          // Swipe right - go back
          runOnJS(handleBack)();
        } else if (translationX < 0 && !isLastStep && canSwipeNext.value) {
          // Swipe left - go next
          runOnJS(handleNext)();
        }
      }
